...
export default function RekapTkaPage() {
  ...
  const [filterStatus, setFilterStatus] = useState("BELUM_MERESPONS");
  ...
  const activeClass = filterKelas !== "ALL" ? classes.find((c) => c.nama === filterKelas) : null;
  const totalCount = classes.length > 0
    ? (activeClass ? activeClass.totalSiswa : classes.reduce((sum, c) => sum + (c.totalSiswa || 0), 0))
    : students.length;
  const ikutCount = classes.length > 0
    ? (activeClass ? activeClass.ikut : classes.reduce((sum, c) => sum + (c.ikut || 0), 0))
    : students.filter((s) => s.statusTka === "IKUT").length;
  const tidakIkutCount = classes.length > 0
    ? (activeClass ? activeClass.tidakIkut : classes.reduce((sum, c) => sum + (c.tidakIkut || 0), 0))
    : students.filter((s) => s.statusTka === "TIDAK_IKUT").length;
  const belumCount = classes.length > 0
    ? (activeClass ? activeClass.belumRespons : classes.reduce((sum, c) => sum + (c.belumRespons || 0), 0))
    : students.filter((s) => s.statusTka === "BELUM_MERESPONS").length;

  // Pie chart data
  const pieChartData = [
    { name: "Bersedia Ikut", value: ikutCount, fill: "#2563eb" },
    { name: "Tidak Ikut", value: tidakIkutCount, fill: "#f43f5e" },
    { name: "Belum Merespons", value: belumCount, fill: "#f59e0b" },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center justify-between text-[11px] font-bold text-slate-400 mb-1">
            <span className="uppercase">Total Siswa Terdaftar</span>
            <School className="w-4 h-4 text-slate-400" />
          </div>
          <div className="text-2xl font-black text-slate-900">{totalCount} Siswa</div>
        </button>
        ...
        >
          <div className="flex items-center justify-between text-[11px] font-bold text-slate-500 mb-1">
            <span className="flex items-center gap-1.5 font-bold text-amber-900">
              ...
              <span className="text-[9px] px-1.5 py-0.5 rounded-md bg-amber-200 text-amber-950 font-black tracking-wider uppercase">Default</span>
            </span>
            <Clock className="w-4 h-4 text-amber-600" />
          </div>
          <div className="text-2xl font-black text-amber-900">{belumCount} Siswa</div>
        </button>
      </div>
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-slate-50/50">
          <div>
            <h3 className="font-extrabold text-slate-900 text-sm flex items-center gap-2">
              ...
              <span>
                {filterStatus === "BELUM_MERESPONS"
                  ? "Daftar Siswa Belum Konfirmasi TKA (Perlu Tindak Lanjut)"
                  : filterStatus === "IKUT"
                  ? "Daftar Siswa Bersedia Ikut TKA"
                  : filterStatus === "TIDAK_IKUT"
                  ? "Daftar Siswa Tidak Ikut TKA"
                  : "Daftar Seluruh Siswa"}
                {filterKelas !== "ALL" ? ` — Kelas ${filterKelas}` : " — Semua Kelas"}
              </span>
            </h3>
            <p className="text-[11px] text-slate-500 mt-0.5">
              {filterStatus === "BELUM_MERESPONS"
                ? "Secara default menampilkan siswa yang belum melakukan konfirmasi di semua kelas untuk percepatan tindak lanjut."
                : "Menampilkan daftar siswa terfilter sesuai kriteria status dan rombel kelas yang dipilih."}
            </p>
          </div>
          {(filterStatus !== "BELUM_MERESPONS" || filterKelas !== "ALL" || searchTerm) && (
            <button
              onClick={() => {
                setFilterStatus("BELUM_MERESPONS");
                setFilterKelas("ALL");
              ...
              className="text-xs text-blue-600 hover:text-blue-800 font-bold self-start sm:self-auto flex items-center gap-1 cursor-pointer"
            >
              <X className="w-3.5 h-3.5" />
              <span>Kembali ke Default (Belum Konfirmasi)</span>
            </button>
          )}
        </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full sm:w-auto">
            ...
            <div className="flex items-center gap-1 text-xs text-slate-500 font-semibold">
              ...
              >
                ...
                <option value="BELUM_MERESPONS">Belum Merespons (Default)</option>
              </select>
            </div>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-4 py-8 text-center text-slate-400">
                    {filterStatus === "BELUM_MERESPONS"
                      ? "Tidak ada siswa yang belum merespons. Semua siswa telah melakukan konfirmasi TKA!"
